    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (command.getName().toLowerCase()) {
            ...
            case "spawn" -> {
                ...
                plugin.msg().send(player, "<green>Welcome to spawn!");
            }
            case "setspawn" -> {
                if (!player.hasPermission("apollo.admin")) {
                    plugin.msg().send(player, "<red>You don't have permission.");
                    return true;
                }
                plugin.setSpawn(player.getLocation());
                plugin.msg().send(player, "<green>Server spawn set to your location.");
            }
